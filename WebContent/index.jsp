						...
						<c:forEach items="${sessionScope.listsn }" var="itemson">
							<c:choose>
								<c:when test="${item.id eq itemson.parentId}">
									<dd>
										<a href="ProductServlet?cid=${itemson.id}&ps=showProduct">${itemson.name}</a>
									</dd>
								</c:when>
							</c:choose>
						</c:forEach>
				...
				<ul class="product clearfix">
					<c:forEach items="${page.pageList }" var="product">
						<li>
							<dl>
								<dt>
									<a href="ProductServlet?ps=detail&id=${product.id }"
										target="_self"><img src="${product.fileName}" /></a>
								</dt>
								<dd class="title">
									<a href="ProductServlet?ps=detail&id=${product.id }"
